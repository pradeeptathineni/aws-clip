...
// TestMain dispatches hermetic subprocesses so tests exercise real process boundaries.
...
// Set distinguishes an unset variable from an explicitly empty value
...
// Environment controls keep the fake deterministic and offline
func fakeAWS() {
	arguments := os.Args[1:]
...
// Call traces may contain sensitive arguments, so fixtures remain owner-readable
...
// Fall back to copying when the platform or filesystem disallows symlinks
